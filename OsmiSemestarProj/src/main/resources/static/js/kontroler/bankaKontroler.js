...
bankaKontroler.controller('bankaCtrl', function($scope, klijentServis, bankaServis, $window, $location, klijentServis) {
	
	$scope.bankaBankarskog = [];
	$scope.bankarskiSluzbenik = new Object();
	
	klijentServis.ucitajUlogovanogKorisnika().success(function(data) {
		$scope.bankarskiSluzbenik = data.korisnik
		bankaServis.aboutBanka($scope.bankarskiSluzbenik).success ( function (data1){
			$scope.bankaBankarskog = data1;
